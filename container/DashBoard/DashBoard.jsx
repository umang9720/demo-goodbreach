...
//     setLoading(true);
//     const response = await apiCall('/api/goals', 'POST', { userId: user.id }, token);
//     setLoading(false);
...
//       ]}
//     >
//       <Text style={styles.habitText}>{status === 'success' ? '✓' : '✗'}</Text>
//     </View>
//   );

//   const ProgressBar = ({ progress, color = '#FB923C' }) => (
...
import GoalCard from '../../container/Goal/GoalCard';
 ...
 useEffect(() => {
  const fetchGoals = async () => {
    try {
      ...
      const response = await api('/goals/user', 'POST', { userId: user._id }, user.token);
      console.log('All Goals:', JSON.stringify(response.data.data, null, 2));

      if (response?.status === 201 && Array.isArray(response?.data?.data)) {
        const activeGoals = response.data.data.filter(goal => goal.status === 'active');
  ...
  return (
    <View style={styles.container}>
      <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10 }}>Your Active Goals</Text>
      ...
      <Button title="Create Goal" onPress={() => navigation.navigate('CreateGoal')} />
